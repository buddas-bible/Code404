...
namespace BJEngine
{
	...
	void SoftBody::Update()
	{
		...
		if (Input::GetInstance().ReleasedKey(VK_LEFT) ||
			Input::GetInstance().ReleasedKey(VK_RIGHT))
		{
			move.x = 0.f;
		}
		if (Input::GetInstance().ReleasedKey(VK_UP) ||
			Input::GetInstance().ReleasedKey(VK_DOWN))
		{
			move.y = 0.f;
		}
